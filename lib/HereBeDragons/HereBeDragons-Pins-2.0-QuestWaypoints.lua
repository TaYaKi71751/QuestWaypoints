...
local MAJOR, MINOR = "HereBeDragons-Pins-2.0-QuestWaypoints", 12
...
-- custom
CreateFrame("Frame", "MinimapObjects", Minimap)
MinimapObjects:SetAllPoints()	
MinimapObjects:SetClipsChildren(true)		

pins.selectedQuest = {} 
function pins:setSelectedQuest(selectedQuest)
	pins.selectedQuest = selectedQuest
end
...
local function drawMinimapPin(pin, data)
	...
	-- blob handle
	if pin.pinTemplate and pin.pinTemplate == "CustomQuestBlobPinTemplate" then
		pin:Show()
		pin:ClearAllPoints()
		pin:SetPoint("CENTER", MinimapObjects, "CENTER", diffX * minimapWidth, -diffY * minimapHeight)
	else
...
-- custom
local function updatePOIPin(pin)
	if pins.selectedQuest.mapID and pins.selectedQuest.mapID == C_Map.GetBestMapForUnit("player") then
		local zoom = pins.Minimap:GetZoom()
		local size = minimap_size[indoors][zoom];
		local zoneWidth, zoneHeight = HBD:GetZoneSize(C_Map.GetBestMapForUnit("player"))
		pin:SetSize(zoneWidth / size * Minimap:GetWidth() , zoneHeight / size * Minimap:GetHeight() );
		
		--print("update minimap poi")
		--pin:DrawNone()
...
function pins:AddMinimapIconMap(ref, icon, uiMapID, x, y, showInParentZone, floatOnEdge)
	...
	-- custom
	if icon.pinTemplate and icon.pinTemplate == "CustomQuestBlobPinTemplate" then
		updatePOIPin(icon)
	end			
end
